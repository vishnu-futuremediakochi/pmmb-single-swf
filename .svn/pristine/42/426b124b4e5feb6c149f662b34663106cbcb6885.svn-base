package {
    ...
    public class Bike2 extends Sprite {
        ...
        public function Bike2(posx:int,posy:int):void {
            x=posx;
            y = posy;
			rposx = posx;
			rposy = posy;
        ...
        public function on_key_down(e:KeyboardEvent):void {
            if (e.keyCode==38) {
                accelerate = true;
				trace('accelerate=' + accelerate);
			/*	if (isEngineSoundHigh == false)
				{
		...
		function playEngineHighSound(evt:Event)
		{
			...
			if (restartTimer)
			{
				...
				{
					restartTimer.removeEventListener(TimerEvent.TIMER_COMPLETE, playEngineHighSound);			
				}
			}
			
			restartTimer = new Timer(rbEngineHigh.length -100 , 1);			
			restartTimer.start();			
			restartTimer.addEventListener(TimerEvent.TIMER_COMPLETE, playEngineHighSound);
		}
